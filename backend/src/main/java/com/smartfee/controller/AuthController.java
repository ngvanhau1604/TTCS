...
public class AuthController {
    ...
    public ResponseEntity<?> getCurrentUser(Authentication authentication) {
        return authService.getUserByUsername(authentication.getName())
                .map(user -> ResponseEntity.ok(Map.of(
                        ...
                        "apartmentCode", user.getApartmentCode() != null ? user.getApartmentCode() : "",
                        "role", user.getRole()
                )))
    ...
    @PutMapping("/me")
    public ResponseEntity<?> updateCurrentUser(@RequestBody Map<String, String> body, Authentication authentication) {
        String fullName = body.get("fullName");
        String phoneNumber = body.get("phoneNumber");
        try {
            User updated = authService.updateUserProfile(authentication.getName(), fullName, phoneNumber);
            return ResponseEntity.ok(Map.of(
                    "userId", updated.getUserId(),
                    "username", updated.getUsername(),
                    "fullName", updated.getFullName(),
                    "phoneNumber", updated.getPhoneNumber(),
                    "apartmentCode", updated.getApartmentCode() != null ? updated.getApartmentCode() : "",
