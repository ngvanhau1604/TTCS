...
public class AuthService {
    ...
    public User updateUserProfile(String username, String fullName, String phoneNumber) {
    ...
    public User updateResidentProfileByAdmin(Integer userId, String fullName, String phoneNumber, String roomNumber) {
        User user = userRepository.findById(userId)
                .orElseThrow(() -> new InvalidDataException("Người dùng không tồn tại"));
        ...
        if (roomNumber != null && !roomNumber.isBlank()) {
            String oldRoom = user.getApartmentCode();
            String newRoom = roomNumber.trim().toUpperCase();
            if (!newRoom.equalsIgnoreCase(oldRoom)) {
                // Giải phóng căn hộ cũ
                if (oldRoom != null && !oldRoom.isBlank()) {
                    apartmentRepository.findByRoomNumber(oldRoom).ifPresent(apt -> {
                        if (apt.getOwner() != null && apt.getOwner().getUserId().equals(userId)) {
                            apt.setOwner(null);
                ...
                // Gán căn hộ mới
                Apartment newApt = apartmentRepository.findByRoomNumber(newRoom)
                        .orElseThrow(() -> new InvalidDataException("Mã căn hộ mới không tồn tại"));
                if (newApt.getOwner() != null && !newApt.getOwner().getUserId().equals(userId)) {
                    throw new InvalidDataException("Căn hộ mới đã có cư dân được gán");
                }
                user.setApartmentCode(newRoom);
                newApt.setOwner(user);
                apartmentRepository.save(newApt);
            }
        }
        return userRepository.save(user);
    }
