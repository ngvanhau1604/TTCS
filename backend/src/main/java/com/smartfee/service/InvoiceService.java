...
public class InvoiceService {
    ...
    public int generateMonthlyInvoices(int month, int year) {
        ...
        for (Apartment apt : apartments) {
            ...
            // Tìm hóa đơn cũ để cập nhật hoặc tạo mới
            Invoice invoice = invoiceRepository.findByApartment_ApartmentIdAndBillingMonth(
                    apt.getApartmentId(), billingMonth).orElse(null);

            // Nếu hóa đơn đã tồn tại nhưng không có chỉ số mới được ghi nhận cho tháng này, giữ nguyên không ghi đè về 0
            if (invoice != null && reading == null) {
                continue;
            }
            ...
            invoice.setStatus("PENDING"); // Đặt lại trạng thái PENDING khi tính phí/tính lại phí để ghi nợ cư dân
